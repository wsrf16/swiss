...
func mustTestableIptables() []*IPTables {
	ipt, err := New()
	if err != nil {
		panic(log.Sprintf("New failed: %v", err))
	}
	ip6t, err := NewWithProtocol(ProtocolIPv6)
	if err != nil {
		panic(log.Sprintf("NewWithProtocol(ProtocolIPv6) failed: %v", err))
	}
...
func TestChain(t *testing.T) {
	for i, ipt := range mustTestableIptables() {
		t.Run(log.Sprint(i), func(t *testing.T) {
			runChainTests(t, ipt)
		})
	}
}
...
func TestListById(t *testing.T) {
	...
	for _, tt := range testCases {
		t.Run(log.Sprintf("Checking rule with id %d", tt.id), func(t *testing.T) {
...
func TestCustom(t *testing.T) {
	tables, err := New()
	if err != nil {
		t.Fatalf("Failed to new up an IPtables intance. ERROR: %v", err)
	}

	list, err := tables.List("nat", "OUTPUT")
	if err != nil {
		t.Fatalf("Failed to list. ERROR: %v", err)
	}
	t.Log(list)

	err = tables.Append("nat", "OUTPUT", strings.Split("--destination 128.0.0.1 -p tcp --dport 8080 -j DNAT --to-destination 127.0.0.1:8080", " ")...)
	if err != nil {
		t.Fatalf("", nil, "Failed to append. ERROR: %v", err)
	}
}
