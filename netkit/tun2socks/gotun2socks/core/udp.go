...
func getUDPPacket() *udpPacket {
	return udpPacketPool.Get().(*udpPacket)
}

func putUDPPacket(pkt *udpPacket) {
	packet.PutIPv4(pkt.ip)
	packet.PutUDP(pkt.udp)
	if pkt.mtuBuf != nil {
		PutMTU(pkt.mtuBuf)
	}
	pkt.mtuBuf = nil
...
func (ut *udpConnTrack) run() {
	...
	var e error
	for i := 0; i < 2; i++ {
		ut.socksConn, e = dialSocksServer(ut.socksAddress)
		if e != nil {
...
func (t2s *Tun2Socks) isDNS(remoteIP string, remotePort uint16) bool {
	...
	for _, s := range t2s.dnsAddresses {
		if s == remoteIP {
			return true
		}
	}
