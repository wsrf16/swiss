...
func getTCPPacket() *tcpPacket {
	return tcpPacketPool.Get().(*tcpPacket)
}

func putTCPPacket(pkt *tcpPacket) {
	packet.PutIPv4(pkt.ip)
	packet.PutTCP(pkt.tcp)
	if pkt.mtuBuf != nil {
		PutMTU(pkt.mtuBuf)
	}
	pkt.mtuBuf = nil
...
func copyTCPPacket(raw []byte, ip *packet.IPv4, tcp *packet.TCP) *tcpPacket {
	iphdr := packet.GetIPv4()
	tcphdr := packet.GetTCP()
	pkt := getTCPPacket()
...
func (tt *tcpConnTrack) stateClosed(syn *tcpPacket) (continu bool, release bool) {
	var err error
	for i := 0; i < 2; i++ {
		tt.socksConn, err = dialSocksServer(tt.socksAddress)
		if err != nil {
			log.Printf("fail to connect SOCKS proxy: %s", err)
		} else {
			// no timeout
