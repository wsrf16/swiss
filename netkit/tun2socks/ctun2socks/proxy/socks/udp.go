...
//func NewUDPHandlerOld(proxyHost string, proxyPort uint16, timeout time.Duration) core.UDPConnHandler {
//    return &udpHandler{
//        proxyHost:   proxyHost,
//        proxyPort:   proxyPort,
...
func (h *udpHandler) fetchUDPInput(conn core.UDPConn, input net.PacketConn) {
	...
	for {
		...
		if err != nil {
			logo.Wf("write local failed: %v", err)
			return
		}
	}
}
...
func (h *udpHandler) connectInternal(conn core.UDPConn, dest string) error {
	c, err := net.DialTimeout("tcp", h.proxyAddress, 4*time.Second)
	if err != nil {
		return err
	}
	...
	logo.If("new proxy connection to %v", dest)
