...
func BlockOutsideDNS(tunName string) error {
	...
	logo.Df("Added filter to block all IPv6 traffic")
	...
	logo.Df("Added filter to allow all IPv4 traffic to %v", tunName)
	...
	logo.Df("Added filter to block all IPv4 UDP traffic targeting port 53")
